...
impl EdgeCaseCategoryType {
    pub fn display_name(&self) -> &'static str {
        match self {
            EdgeCaseCategoryType::NetworkTimeouts => "Network & Timeouts",
...
pub fn evaluate_invest_scorecard(story: &Story) -> InvestScorecard {
    ...
    criteria.push(InvestCriterionResult {
        ...
        recommendation: if independent_passed {
            ...
            Some(
                "Decouple external dependencies or agree on interface contracts before voting."
                    .to_string(),
            )
        },
    });
    ...
    let negotiable_obs = if negotiable_passed {
        "Focuses on user/system outcomes while preserving implementation flexibility for the team."
            .to_string()
    } else {
        let issue = format!(
    ...
    let valuable_obs = if valuable_passed {
        ...
        let issue =
            "Missing explicit user or business value statement (e.g. 'So that...').".to_string();
        issues.push(issue.clone());
        issue
    };
    ...
    let summary = if total_score == 100 {
        ...
        "Low readiness score; missing key criteria (e.g. acceptance criteria or value statements)."
            .to_string()
    };
...
pub fn analyze_3axis_complexity(story: &Story) -> ComplexitySummary {
    ...
    let data_models = if !matched_data.is_empty() {
        ...
        "Standard in-memory / UI state mutations without dedicated database schema alterations."
            .to_string()
    };
    ...
    let blast_radius = if !matched_blast.is_empty() {
        ...
        "Isolated component change with low regression risk and localized failure blast radius."
            .to_string()
    };
...
pub fn generate_4category_edge_cases(story: &Story) -> Vec<EdgeCaseItem> {
    ...
    let title_desc = story.title_and_description_lowercase();

    // 1. Error & Failure States
    let (err_title, err_desc) = if title_desc.contains("webhook")
        || title_desc.contains("api")
        || title_desc.contains("tracker")
    {
        (
    ...
    } else if title_desc.contains("export") || title_desc.contains("csv") {
        (
    ...
    items.push(EdgeCaseItem {
        id: format!("{}-ec-net", story.id),
        category: EdgeCaseCategoryType::NetworkTimeouts,
        category_name: EdgeCaseCategoryType::NetworkTimeouts
            .display_name()
            .to_string(),
        title: err_title,
    ...
    let (empty_title, empty_desc) = if title_desc.contains("list")
        || title_desc.contains("backlog")
        || title_desc.contains("search")
    {
        (
            "Empty dataset (0 items returned)".to_string(),
            "Clear zero-state illustration and instructions when no records or matches exist."
                .to_string(),
        )
    } else if title_desc.contains("field")
        || title_desc.contains("input")
        || title_desc.contains("title")
    {
        (
    ...
    items.push(EdgeCaseItem {
        ...
        category_name: EdgeCaseCategoryType::EmptyBoundary
            .display_name()
            .to_string(),
        title: empty_title,
    ...
    let (race_title, race_desc) = if title_desc.contains("vote")
        || title_desc.contains("real-time")
        || title_desc.contains("websocket")
    {
        (
            "Simultaneous multi-tab or multi-user mutations".to_string(),
            "Handling concurrent vote submissions or state flips across multiple tabs for the same Estimator.".to_string(),
        )
    } else if title_desc.contains("migration") || title_desc.contains("cache") {
        (
        ...
        (
            ...
            "Debouncing rapid user actions and preventing out-of-order response reconciliation."
                .to_string(),
        )
    };
    ...
    items.push(EdgeCaseItem {
        ...
        category_name: EdgeCaseCategoryType::ConcurrencyRaces
            .display_name()
            .to_string(),
        title: race_title,
    ...
    let (perm_title, perm_desc) = if title_desc.contains("auth")
        || title_desc.contains("session")
        || title_desc.contains("token")
    {
        (
            "Expired session token or invalidated credentials".to_string(),
            "Automatic refresh attempt or clean redirect to login without corrupting ongoing work."
                .to_string(),
        )
    } else if title_desc.contains("facilitator") || title_desc.contains("role") {
        (
    ...
    items.push(EdgeCaseItem {
        ...
        category_name: EdgeCaseCategoryType::PermissionsAccess
            .display_name()
            .to_string(),
        title: perm_title,
...
Perform the following Story refinement steps:
...
3. 4-Category Edge Cases:
   - Network & Timeouts (rate limits, network drops)
